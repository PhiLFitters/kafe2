...
class XYFitEnsemble(FitEnsembleBase):
    ...
    :py:meth:`~kafe2.fit.XYFitEnsemble.add_error` or
    :py:meth:`~kafe2.fit.XYFitEnsemble.add_matrix_error`
    methods.

    Once an uncertainty model is provided, the fit ensemble can be generated by using the
    :py:meth:`~kafe2.fit.XYFitEnsemble.run` method.
    This method starts by generating a pseudo-dataset in such a way that the empirical distribution
    of the data corresponds to the specified uncertainty model.
    It then fits the model to the pseudo-data and extracts information from the fit, such as the
    resulting parameter values or the value of the cost function at the minimum.
    This is repeated a large number of times in order to evaluate the whole ensemble in a
    statistically meaningful way.

    The ensemble result can be visualized by using the
    :py:meth:`~kafe2.fit.XYFitEnsemble.plot_results` method.
    ...
    def _initialize_ensemble_variables(self):
        ...
        if "cost" in self._requested_results:
            self._ensemble_variables["cost"] = EnsembleVariable(
                ...
                # FIXME: assume chi2 for all cost functions -> change
                distribution=scipy.stats.chi2,
