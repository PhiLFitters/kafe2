...
class FitBase(FileIOMixin, object):
    ...
    def __init__(
        ...
        if self.MODEL_FUNCTION_TYPE is None:
            ...
            if not self.RESERVED_NODE_NAMES.isdisjoint(
                ...
                raise ValueError(
                    "The following names are reserved and cannot be used as model function "
                    "arguments: %r" % (_invalid_args,)
                )
    ...
    def _set_new_data(self, new_data):
        """Private method called by the :py:attr:`~data` setter. Must be overwritten for each
        derived class."""
        pass
    ...
    def _report_data(self, output_stream, indent, indentation_level):
        ...
        :param int indentation_level: How many times the indent str should be placed before each
            line.
        """
        pass
    ...
    def _report_fit_results(
        ...
        :param int indentation_level: How many times the indent str should be placed before each
            line.
        :param bool asymmetric_parameter_errors: If asymmetric parameter uncertainties should be
            used instead of symmetric uncertainties.
        """
        output_stream.write(indent * indentation_level + "###############\n")
        ...
        if not self.did_fit:
            output_stream.write(
                indent * (indentation_level + 1) + "WARNING: No fit has been performed as of yet. "
    ...
    def _update_parameter_formatters(self, update_asymmetric_errors=False):
        ...
        :param bool update_asymmetric_errors: If the asymmetric parameter uncertainties should be
            updated as well.
        """
    ...
    def model_function(self):
        """The wrapped model function as a :py:obj:`~kafe2.fit._base.ModelFunctionBase` or derived
        object.
    ...
    @property
    def has_errors(self):
        """:py:obj:`True` if at least one uncertainty source is defined for either the data or the
        model.
    ...
    def set_parameter_values(self, **param_name_value_dict):
        """Set the fit parameters to new values. Valid keyword arguments are the names of the
        declared fit parameters.
    ...
    def fix_parameter(self, name, value=None):
        ...
        :param float or None value: The value to be given to the fixed parameter. If :py:obj:`None`
            the current value from :py:attr:`~parameter_values` will be used.
        """
    ...
    def release_parameter(self, name):
        """Release a fixed parameter so that its value once again changes when calling
        :py:meth:`~do_fit()`.
    ...
    def add_matrix_parameter_constraint(
        ...
        In other words the first index must belong to the first value, the first row/column in the
        matrix, etc.
        ...
        :param matrix: The matrix that defines the correlation between the parameters.
            By default interpreted as a covariance matrix.
            Can also be interpreted as a correlation matrix by setting **matrix_type**.
            Must be of shape (N, N).
        :type matrix: typing.Iterable[float]
        :param matrix_type: Either ``'cov'`` or ``'cor'``.
            Defines whether the matrix should be interpreted as a covariance matrix or as a
            correlation matrix.
        ...
        :param relative: Whether the covariance matrix/the uncertainties should be interpreted as
            relative to **values**.
        :type relative: bool
        """
    ...
    def add_parameter_constraint(self, name, value, uncertainty, relative=False):
        ...
        :param float uncertainty: The uncertainty with which the parameter should be constrained to
            the given value.
    ...
    def get_matching_errors(self, matching_criteria=None, matching_type="equal"):
        ...
        .. note::
            The error objects contained in the dictionary are not copies, but the original error
            objects.
            Modifying them is possible, but not recommended.
            If you do modify any of them, the changes will not be reflected in the total error
            calculation until the error cache is cleared.
            This can be done by calling the private dataset method
        ...
        :param matching_criteria: Key-value pairs specifying matching criteria.
            The resulting error array will only contain error objects matching *all* provided
            criteria.
        ...
        :param matching_type: How to perform the matching.
            ...
            stored value.
            If ``'regex'``, the value in **matching_criteria** is interpreted as a regular
            expression and is matched against the stored value.
        :type matching_type: str
        :return: Dict mapping error name to :py:obj:`~kafe2.core.error.GaussianErrorBase`-derived
            error objects.
    ...
    def add_error(
        ...
        :param name: Unique name for this uncertainty source. If :py:obj:`None`, the name of the
            error source will be set to a random alphanumeric string.
        ...
        :param relative: If :py:obj:`True`, **err_val** will be interpreted as a *relative*
            uncertainty.
        :type relative: bool
        :param reference: Either ``'data'`` or ``'model'``. Specifies which reference values to use
            when calculating absolute errors from relative errors.
        :type reference: str
    ...
    def disable_error(self, err_id):
        """Temporarily disable an uncertainty source so that it doesn't count towards calculating
    ...
    def enable_error(self, err_id):
        """(Re-)Enable an uncertainty source so that it counts towards calculating the total
        uncertainty.
    ...
    def do_fit(self, asymmetric_parameter_errors=False):
        ...
        :param bool asymmetric_parameter_errors: If :py:obj:`True`, calculate asymmetric parameter
            errors.
    ...
    def report(
        ...
        :param asymmetric_parameter_errors: If :py:obj:`True`, use two different parameter errors
