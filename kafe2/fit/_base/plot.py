...
class Plot:
        ...
        if self._multifit is None:
            for _plot_adapter in _plot_adapters:
                if not _plot_adapter._fit.did_fit and not _plot_adapter.from_container:
                    warnings.warn(
                        "No fit has been performed for {}. "
                        "Did you forget to run fit.do_fit()?".format(_plot_adapter._fit)
                    )
    ...
    def _adjust_plot_ranges(self, plot_results):
        """set the x and y ranges (all axes) to the total data range reported by the plot
        adapters"""
    ...
    def plot(
        ...
        :param asymmetric_parameter_errors: if ``True``, parameter errors in fit results will be
            asymmetric
        :param ratio: if ``True``, a secondary plot containing data/model ratios is shown below the
            main plot
    ...
    def get_keywords(self, plot_type):
        """Retrieve keyword arguments for plots with type `plot_type` as they would be used when
        calling `plot`.
        ...
        :param plot_type: keyword identifying the plot type for which to set a custom keyword
            argument
        :type plot_type: str

        :return: list of dictionaries (one per fit instance) containing plot keywords and their
            values
        :rtype: list of dict
        """
    ...
    def set_keywords(self, plot_type, keyword_spec):
        ...
        * ``'__default__'``: the customized value will be replaced by the default value.
        ...
        .. note::
            ...
            methods as specified. Incorrect or incompatible keywords may be ignored or lead to
            errors.
        ...
        :param plot_type: keyword identifying the plot type for which to set a custom keyword
            argument
        :type plot_type: str
        :param keyword_spec: specification of dictionaries containing the keyword arguments to use
            for fit.
            Can be either a list of dictionaries with a length corresponding to the number of `fit`
            objects managed by this `Plot` instance, or a list of tuples of the form
            ``(index, dict)``, where ``index``  denotes the index of the `fit` object for which the
            dictionary `dict` should be used.
    ...
    def customize(self, plot_type, keyword, values):
        ...
        * ``'__skip__'``: the keywords for this `fit` will not be changed.
        ...
        :type keyword: str
        :param values: values that the keyword argument should take for each fit. Can be a list of
            values with a length corresponding to the number of `fit` objects managed by this `Plot`
            instance, or a list of tuples of the form ``(index, value)``
