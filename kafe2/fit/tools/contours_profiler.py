...
class ContoursProfiler(object):
    ...
    _DEFAULT_PLOT_PROFILE_ARROW_KWARGS = dict(
        ...
        shrinkA=0,
        shrinkB=0,
        horizontal_arrow_length=0.05,
        arrowstyle=mpl.patches.ArrowStyle("fancy", head_length=6, head_width=6, tail_width=0.01),
        vertical_x_text=0.005,
        vertical_y_text=0.005,
        horizontal_x_text=0.015,
    ...
    def _get_profile(
        ...
        return self._fit._fitter.profile(parameter, low, high, sigma, cl, points, subtract_min, arrows)
    ...
    def _plot_profile_arrow(target_axes, x_span, y_span, par_formatter, side, x, y, cl):
        ...
        _par_text = par_formatter.get_formatted(value=x, with_name=True, with_errors=False, format_as_latex=True)
        ...
        format_str = _float_template if 100 * cl < 0.1 else ".2f"
        target_axes.text(_x_text, _y_text, rf"${100*cl:{format_str}}\%$", horizontalalignment=_alignment)
    ...
    def get_profile(self, parameter, low=None, high=None, sigma=None, cl=None, points=None, subtract_min=None):
        """
        ...
        """
        return self._get_profile(parameter, low, high, sigma, cl, points, subtract_min, arrows=False)[0]
    ...
    def get_contours(self, parameter_1, parameter_2, smoothing_sigma=None):
        ...
        for _cl_obj in self._contour_kwargs["confidence_levels"]:
            ...
            _cont = self._fit._fitter.contour(parameter_1, parameter_2, sigma=_cl_obj.sigma, **_contour_method_kwargs)
    ...
    def plot_profile(
        ...
        with rc_context(kafe2_rc):
            ...
            (_x, _y), _arrow_specs = self._get_profile(parameter, low, high, sigma, cl, arrows=True)
            ...
            if show_fit_minimum:
                _minimum_artist = self._plot_minimum(_axes, x=_par_val, y=_y_offset, xerr=_par_err, yerr=None, label="fit minimum")
            ...
            _axes.set_ylabel(
                self._cost_function_formatted_name if not self._profile_kwargs["subtract_min"] else "$\\Delta$%s" % self._cost_function_formatted_name
            )
    ...
    def plot_contours(
        ...
        with rc_context(kafe2_rc):
            ...
            if naming_convention.lower() == "cl":
                ...
                raise ValueError("Unknown contour naming convention '%s'! " "Must be one of: ('cl', 'sigma')" % (naming_convention,))
            ...
            for _cl_contour_pair, _prop_cycler in zip(_cl_contour_pairs, rcParams["axes.prop_cycle"]):
                ...
                if _contour_xy is not None:
                    ...
                    _artists = self._plot_contour_xy(_axes, _contour_xy, label=_label, contour_color=_prop_cycler["color"])
    ...
    def plot_profiles_contours_matrix(
        ...
        with rc_context(kafe2_rc):
            ...
            if _par_names is None:
                ...
                if _fixed_pars:
                    raise ValueError("The following parameters cannot be profiled because they are fixed: " f"{_fixed_pars}")
            ...
            if show_grid_for is not None and show_grid_for not in _show_spec_options:
                raise ValueError("Unknown specification '%s' for 'show_grid_for'. " "Expected: one of %r" % (show_grid_for, _show_spec_options))
            if show_ticks_for is not None and show_ticks_for not in _show_spec_options:
                raise ValueError("Unknown specification '%s' for 'show_ticks_for'. " "Expected: one of %r" % (show_ticks_for, _show_spec_options))
            if show_fit_minimum_for is not None and show_fit_minimum_for not in _show_spec_options:
                raise ValueError(
                    "Unknown specification '%s' for 'show_fit_minimum_for'. " "Expected: one of %r" % (show_fit_minimum_for, _show_spec_options)
                )
    ...
    def save(self, fname=None, figures="all", *args, **kwargs):
        ...
        if figures == "all":
            ...
            raise TypeError(f"figures must be either 'all', int or iterable of int, but received type" f"{type(figures)}")
