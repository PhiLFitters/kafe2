...
def Fit(data=None, model_function=None, minimizer=None, **kwargs):
    """A convenience wrapper for simple fit creation. For more control over the fit creation use
    the corresponding Fit classes.

    :param data: A :py:obj:`~kafe2.fit._base.DataContainerBase`-derived data container, containing
    ...
    :param minimizer: The minimizer backend to use for the fit. This can be ``'scipy'``,
        ``'iminuit'`` or ``'tminuit'``, depending on the installed backends.
    :type minimizer: str or None
    :param kwargs: Any further keyword arguments for the according fit types. For more information,
