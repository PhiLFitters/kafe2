...
def broadcast_to_shape(array, shape, scheme="default"):
    ...
    raise EnsembleError(
        "Cannot broadcast array to ensemble shape: unknown scheme '{}'! "
        "Available: {}".format(
            scheme,
            (
                "default",
                ...
                "expand_right",
                "expand_left",
                "expand_right_successive",
                "expand_left_successive",
            ),
        )
    )
...
class EnsembleVariable(object):
    """
    Object for storing a finite sample of realizations of a single (possibly multidimensional)
    random variable, forming a statistical ensemble.
    """
    ...
    def __init__(
        ...
        :param ensemble_array: a statistical ensemble containing all realizations of the random
            variable.
        ...
        :typle variable_shape: tuple of int (pass an empty `tuple()` or list `[]` for a scalar
            variable)
        ...
        :param distribution_parameters:  mapping of distribution parameter names to values. If
            values are sequences/arrays, the shape must match the `variable_shape`
    ...
    @property
    def values(self):
        """A (possibly) multidimensional array containing all realization of the ensemble
        variable."""
        return self._array
...
class EnsembleVariableProbabilityDistribution(object):
        ...
        for _dist_par_name, _dist_par_value in six.iteritems(parameters):
            ...
            if isinstance(_dist_par_value, np.ndarray):
                ...
                if _dist_par_value.shape != self._shape:
                    raise EnsembleError(
                        "Distribution parameter '{}' must be a scalar or have shape "
    ...
    def eval(self, x, x_contains_var_shape=False):
        ...
        :type x: `numpy.ndarray`. If the alternative method above is used, the *first* entries of
            `x.shape` must match the ensemble variable's shape.
...
class EnsembleVariablePlotter(object):
    ...
    try:
        _DEFAULT_PLOT_EXPECTED_MEAN_KWARGS = dict(
            linewidth=1,
            marker="",
            linestyle="--",
    ...
    def __init__(self, ensemble_variable, value_ranges, variable_labels=None, ensemble_label=None):
        """
    ...
    def plot_hist(self, axes_array, show_y_ticks=False):
        ...
        :param axes_array: an array of ``matplotlib`` axes objects in which to perform the plotting.
            The shape of the axes array must match that of the ensemble variable. For scalar
            variables, a single ``Axes`` can be provided.
        ...
        if _dist is not None:
            ...
            _pdf_eval_x = np.apply_along_axis(
                lambda xminmax: np.linspace(xminmax[0], xminmax[1], _pdf_eval_npts),
                -1,
                np.squeeze(self._value_ranges),
            )
            _pdf_eval_y = _dist.eval(_pdf_eval_x, x_contains_var_shape=True)
        ...
        for _index1, _axes in enumerate(np.atleast_2d(axes_array)):
            for _index2, _ax in enumerate(_axes):
                ...
                if _expected_means is not None:
                    ...
                    _ax.annotate(
                        r"$\mu={}$".format(round(_observed_means[_index1, _index2], 2)),
                        xycoords="data",
    ...
    def plot_scatter(self, axes_array):
        ...
        :param axes_array: an array of ``matplotlib`` axes objects in which to perform the plotting.
            The array must be a two-dimensional square (*N* x *N*) array, with *N* being identical
            to the size of the ensemble variable.
