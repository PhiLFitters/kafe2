...
class MultiFit(FitBase):
    ...
    Calling do_fit on the MultiFit will result in a numerical minimization of the sum of the cost
    functions of the individual fits.
    Parameters with the same name will be unified: their value will be the same across all
    individual fits.
    """
    ...
    def __init__(
        ...
        :param minimizer: The minimizer to use for fitting. Either ``'iminuit'``, ``'tminuit'``,
            ``'scipy'`` or ``None``.
    ...
    def model_count(self):
        """The number of model functions contained in the multifit. In most cases one for each
        singular fit.
    ...
    def add_matrix_error(
        ...
        **kwargs
        ...
        :param fits: The indices of the fits to add the error. If ``'all'``, the error is added to
            all fits.
        :type fits: int or collections.Iterable[int] or str
        :param axis: Axis of the individual fits to add the error to. ``y``/``1`` errors are
            treated as regular errors for :py:class:`~kafe2.fit.IndexedFit`.
        :type axis: int or str
        :param name: Unique name for this uncertainty source. If ``None``, the name of the error
            source will be set to a random alphanumeric string.
    ...
    def assign_model_function_expression(self, expression_format_string, fit_index=None):
        ...
        :param fit_index: The index specifying the singular fit. If ``None``, all model functions
            will be changed.
    ...
    def assign_model_function_latex_name(self, latex_name, fit_index=None):
        """Assign a LaTeX-formatted string to be the model function name of one of the individual
        fits.
    ...
    def assign_model_function_latex_expression(
        ...
        """Assign a LaTeX-formatted expression string to the model function of one of the
        individual fits.
        Elements like ``'{par_name}'`` will be replaced automatically with the corresponding LaTeX
        names for the given parameter.
        These can be set with :py:meth:`~assign_parameter_latex_names`.
    ...
    def get_matching_errors(self, fit_index=None, matching_criteria=None, matching_type="equal"):
        ...
        :param fit_index: Index for which fit the method should be executed. If ``None`` the
            function will return the matched errors for all singular fits.
        :type fit_index: int or None
        :param matching_criteria: Key-value pairs specifying matching criteria. The resulting error
            array will only contain error objects matching *all* provided criteria.
    ...
    def report(
        ...
        :type show_data: bool
        :param show_model: If ``True``, print out information about the parametric model for each
            fit.
        ...
        :param asymmetric_parameter_errors: If ``True``, use two different parameter errors for
            up/down directions.
