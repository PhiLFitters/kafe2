...
class TestMatrixGaussianError(unittest.TestCase):
    def setUp(self):
        self.ref_cov_mat = np.array([[0.10, 0.04, 0.29], [0.04, 0.02, 0.42], [0.29, 0.42, 0.33]])
        self.ref_cor_mat = np.array(
            [
                [1.0, 0.89442719, 1.59639746],
                [0.89442719, 1.0, 5.16984262],
                [1.59639746, 5.16984262, 1.0],
            ]
        )
        ...
        self.ref_cov_mat_rel = np.array([[10.0, 2.0, 9.667], [2.0, 0.5, 7.0], [9.667, 7.0, 3.667]])
        ...
        self.ge_cor_rel_wref = MatrixGaussianError(
            self.ref_cor_mat,
            "cor",
            err_val=self.ref_err_val_rel,
            relative=True,
            reference=self.ref_reference,
        )

        # -- ill defined matrices
        self.wrong_cor_mat_diagonal_not_ones = np.array(
            [
                [42.0, 0.89442719, 1.59639746],
                [0.89442719, 1.0, 5.16984262],
                [1.59639746, 5.16984262, 1.0],
            ]
        )

        self.wrong_cor_mat_asymm = np.array(
            [
                [1.0, 42.89442719, 1.59639746],
                [0.89442719, 1.0, 5.16984262],
                [1.59639746, 5.16984262, 1.0],
            ]
        )

        self.wrong_cov_mat_asymm = np.array(
            [[0.10, 42.0, 0.29], [0.04, 0.02, 0.42], [0.29, 0.42, 0.33]]
        )
    ...
    def test_raise_build_from_wrong_cor_mat_diagonal_not_ones(self):
        with self.assertRaises(ValueError):
            MatrixGaussianError(
                self.wrong_cor_mat_diagonal_not_ones,
                "cor",
                err_val=self.ref_err_val,
                reference=self.ref_reference,
            )
...
class TestSimpleGaussianError(unittest.TestCase):
    def setUp(self):
        ...
        self.sge_rel_noref = SimpleGaussianError(
            err_val=self.ref_error_rel,
            corr_coeff=self.ref_corr_coeff,
            relative=True,
            reference=None,
        )
        self.sge_rel_wref = SimpleGaussianError(
            err_val=self.ref_error_rel,
            corr_coeff=self.ref_corr_coeff,
            relative=True,
            reference=self.ref_reference,
        )
        self.sge_abs_neg_ref = SimpleGaussianError(
            err_val=self.ref_error,
            corr_coeff=self.ref_corr_coeff,
            relative=False,
            reference=-self.ref_reference,
        )
        self.sge_rel_neg_ref = SimpleGaussianError(
            err_val=self.ref_error,
            corr_coeff=self.ref_corr_coeff,
            relative=False,
            reference=-self.ref_reference,
