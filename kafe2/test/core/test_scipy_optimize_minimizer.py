...
class TestMinimizerScipyOptimize(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.func3 = fcn_3
        self.m3 = MinimizerScipyOptimize(function_to_minimize=self.func3,
                                  parameter_names=('x', 'y', 'z'),
                                  parameter_values=(1.0, 1.0, 1.0),
                                  parameter_errors=(0.1, 0.1, 0.1), method=self.SCIPY_METHOD)
        self.func1 = fcn_1
        self.m1 = MinimizerScipyOptimize(function_to_minimize=self.func1,
                                  parameter_names=('x',),
                                  parameter_values=(1.0,),
                                  parameter_errors=(0.1,), method=self.SCIPY_METHOD)
        ...
        self._scipy_fmin = opt.minimize(fcn_3_wrapper, (1.,1.,1.), method=self.SCIPY_METHOD)
    ...
    def test_compare_par_values_minimize_fcn3(self):
        ...
        self.assertAlmostEqual(_v[0], 1.23, places=3)
        self.assertAlmostEqual(_v[1], 4.32, places=3)
        self.assertAlmostEqual(_v[2], 9.81, places=3)
    ...
    def test_compare_par_values_minimize_fcn3_limit_unlimit_x(self):
        ...
        self.m3.minimize()
        _v = self.m3.parameter_values
        self.assertAlmostEqual(_v[0], 0.7, places=2)
        self.assertAlmostEqual(_v[1], 4.32, places=2)
        self.assertAlmostEqual(_v[2], 9.81, places=2)

        self.m3.unlimit('x')
        self.m3.minimize()
        _v = self.m3.parameter_values
        self.assertAlmostEqual(_v[0], 1.23, places=2)
        self.assertAlmostEqual(_v[1], 4.32, places=2)
        self.assertAlmostEqual(_v[2], 9.81, places=2)
    ...
    @unittest.skip("'hess_inv' not available for slsqp")
