...
class TestWrapperCallableXY(unittest.TestCase):
    ...
    def _assert_results_equal(self, results_1, results_2):
        ...
        self.assertTrue(np.allclose(list(results_1["parameter_values"].values()),
                                    list(results_2["parameter_values"].values())))
        self.assertTrue(np.allclose(list(results_1["parameter_errors"].values()),
                                    list(results_2["parameter_errors"].values())))
        self.assertTrue(np.allclose(
            results_1["parameter_cor_mat"], results_2["parameter_cor_mat"], rtol=1e-4))
        self.assertTrue(np.allclose(
            results_1["parameter_cov_mat"], results_2["parameter_cov_mat"], rtol=1e-4))
        try:
            self.assertTrue(np.allclose(
    ...
    def _assert_wrapper_equal(
            self, model_function=function_library.linear_model, p0=None, dp0=None,
            rel_to_model=True, limits=None, constraints=None, profile=None, save=True,
            test_matrix=False):
        ...
        _fit.add_error("y", self._y_error_cor_rel, correlation=1.0, relative=True,
                       reference=_error_ref)
        ...
        _result_2 = xy_fit(
            ...
            x_error_cor_rel=self._x_error_cor_rel, y_error_cor_rel=self._y_error_cor_rel,
            errors_rel_to_model=rel_to_model, limits=limits, constraints=constraints,
    ...
    def setUp(self):
        ...
        self._y_data = function_library.linear_model(
            self._x_data + 0.2 * np.random.randn(10), *self._true_pars) + 0.5 * np.random.randn(10)
