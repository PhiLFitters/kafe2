...
class TestCostBuiltin(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self._cost_chi2_cov_mat = (
            self._res.dot(self._cov_mat_inv).dot(self._res) + self._cov_mat_log_det
        )
        self._cost_chi2_pointwise = (
            np.sum((self._res / self._pointwise_errors) ** 2) + self._total_error_squared_log_sum
        )
        self._cost_chi2_no_errors = np.sum(self._res**2)
        self._cost_nll_gaussian = (
            self._cost_chi2_pointwise
            + 2.0 * np.sum(np.log(np.sqrt((2.0 * np.pi)) * self._pointwise_errors))
            - self._total_error_squared_log_sum
        )
        self._cost_nll_poisson = -2.0 * np.sum(
            np.log(self._model_poisson**self._data_poisson)
            - np.log(factorial(self._data_poisson))
            - self._model_poisson
        )
