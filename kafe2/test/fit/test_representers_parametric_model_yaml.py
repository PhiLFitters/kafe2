...
class TestHistParametricModelYamlRepresenter(unittest.TestCase):
    def _assert_read_model_correct(self, read_model, ref_model, errors=False):
        ...
        self.assertTrue(np.all(read_model.data == ref_model.data))
        ...
        self.assertTrue(np.all(read_model.parameters == self._test_model_parameters))
        self.assertTrue(read_model.n_bins == self._test_n_bins)
        self.assertTrue(np.all(read_model.bin_range == self._test_bin_range))
        self.assertTrue(np.all(read_model.bin_edges == self._test_bin_edges))
        self.assertTrue(
            np.allclose(
        ...
        if errors:
            ...
            self.assertTrue(np.allclose(_given_error["err"].error, _read_error["err"].error))
...
class TestXYParametricModelYamlRepresenter(unittest.TestCase):
    ...
    def test_read_from_testfile_stream(self):
        ...
        self.assertTrue(np.allclose(_read_parametric_model.y, self._test_parametric_model.y))
