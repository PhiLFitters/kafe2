...
def hist_model_density(x, mu=14.0, sigma=3.0):
    return stats.norm(mu, sigma).pdf(x)


def hist_model_density_antideriv(x, mu=14.0, sigma=3.0):
...
class TestHistFitBasicInterface(AbstractTestFit, unittest.TestCase):
    ...
    def setUp(self):
        ...
        self._ref_entries = np.array([
            11.47195963, 9.96715403, 19.90275216, 13.65225802, 18.52670233, 17.79707059,
            11.5441954, 18.42331074, 13.3808496, 18.40632518, 13.21694177, 15.34569261,
            9.85164713, 11.56275047, 12.42687109, 9.43924719, 15.03632673, 10.76203991,
            15.16788754, 13.78683534, 10.84686619, 17.29655143, 12.58404203, 13.10171453,
            20.23773075, 15.00936796, 15.38972301, 12.94254624, 19.01920031, 19.67747312,
            14.53196033, 13.78898329, 17.34040707, 17.25328064, 13.64052295, 11.80461841,
            14.21872985, 11.59054406, 20.28243216, 15.43544627, 15.85964469, 13.68948162,
            14.55574162, 15.70198922, 12.85548112, 14.05419407, 15.85172907, 19.2951937,
            12.70652203, 13.47822913, 12.84301792, 13.65407699, 12.81908235, 12.73065184,
            23.34917367, 11.38895625, 9.41615435, 7.24554349, 11.4112971, 14.54736404,
            15.84186607, 15.33880379, 9.93177073, 15.14220967, 15.19535013, 11.00033758,
            11.83671083, 13.02411982, 18.43323913, 12.06272124, 20.57549426, 12.97405382,
            13.35969913, 10.76072424, 16.35912216, 14.52842827, 16.97221766, 13.02791932,
            14.36097113, 16.93839498, 15.60091018, 11.55044489, 14.06915886, 17.64985576,
            11.59865691, 13.41486068, 14.25999508, 10.70887047, 4.08280244, 13.1043861,
            14.62321312, 14.85894591, 14.89235398, 10.60967181, 15.22310211, 10.77853626,
    ...
    def test_update_data(self):
        ...
        # fmt: on
        _fit.data = HistContainer(8, (17, 23), fill_data=_new_entries)
        _fit.add_error(err_val=1.0)

        _ref_data, _ = np.histogram(_new_entries, bins=8, range=(17, 23))
    ...
    def test_reserved_parameter_names_raise(self):
        ...
        with self.assertRaises(ValueError) as _exc:
            HistFit(data=self._ref_hist_cont, model_function=dummy_model, minimizer=self.MINIMIZER)
    ...
    def test_model_no_pars_beside_x_raise(self):
        ...
        self.assertIn("needs at least one parameter besides", _exc.exception.args[0])
