...
class TestIndexedContainerYamlRepresentation(unittest.TestCase):
    def setUp(self):
        _data = [4, 5, 6, 2, 4, 5.0, 3.01e-5, 4.4e45]
        _ndat = len(_data)
        ...
        # self._container.add_matrix_error(
        #     axis='y', err_matrix=np.eye(8) * 0.1, relative=False, matrix_type='covariance')
    ...
    def test_read_from_testfile_stream(self):
        ...
        self.assertTrue(np.allclose(_read_container.data, self._ref_testfile_data))
        self.assertTrue(np.allclose(_read_container.err, self._ref_testfile_err))
        self.assertTrue(
            np.allclose(
        ...
        self.assertEqual(set(_read_container._error_dicts.keys()), self._ref_testfile_error_names)
    ...
    def test_round_trip_with_stringstream(self):
        ...
        self.assertTrue(np.allclose(self._container.data, _read_container.data))
...
class TestXYContainerYamlRepresentation(unittest.TestCase):
    ...
    def test_read_from_testfile_stream(self):
        ...
        self.assertTrue(np.allclose(_read_container.x, self._ref_testfile_x_data))
        self.assertTrue(np.allclose(_read_container.y, self._ref_testfile_y_data))
        self.assertTrue(np.allclose(_read_container.x_err, self._ref_testfile_x_err))
        self.assertTrue(np.allclose(_read_container.y_err, self._ref_testfile_y_err))
        self.assertTrue(
            np.allclose(
    ...
    def test_round_trip_with_stringstream(self):
        ...
        # compare (total) errors and cov mats
        # TODO: compare individual error components -> nontrivial because error ids change...
        self.assertTrue(np.allclose(self._container.x_err, _read_container.x_err))
        self.assertTrue(np.allclose(self._container.y_err, _read_container.y_err))

        self.assertTrue(np.allclose(self._container.x_cov_mat, _read_container.x_cov_mat))
...
class TestHistContainerYamlRepresentation(unittest.TestCase):
    def setUp(self):
        ...
        _nbins = 3
        self._container = HistContainer(n_bins=_nbins, bin_range=(0.0, 10.0), fill_data=_data)

        self._container.add_error(err_val=0.1, name="SC5A_1", correlation=0.5, relative=False)
        self._container.add_error(
            err_val=[i / 10.0 for i in range(1, _nbins + 1)],
            name="SC5A_2",
            correlation=0.5,
            relative=False,
        )
    ...
    def test_read_from_testfile_stream(self):
        ...
        self.assertTrue(np.allclose(_read_container.bin_range, self._ref_testfile_bin_range))
        self.assertTrue(np.allclose(_read_container.bin_edges, self._ref_testfile_bin_edges))
    ...
    def test_round_trip_with_stringstream(self):
        ...
        self.assertTrue(np.allclose(self._container.bin_range, _read_container.bin_range))
        self.assertTrue(np.allclose(self._container.bin_edges, _read_container.bin_edges))
