...
class TestModelFunctionFormatterYamlRepresenter(unittest.TestCase):
    ...
    def _assert_model_function_formatters_equal(self, formatter1, formatter2):
        ...
        for _arg_formatter_1, _arg_formatter_2 in zip(formatter1._arg_formatters, formatter2._arg_formatters):
...
class TestModelParameterFormatterYamlRepresenter(unittest.TestCase):
    def setUp(self):
        self._model_parameter_formatter = ParameterFormatter("phi", name="ph", value=1.571, error=0.1, latex_name=r"\phi")

        self._roundtrip_stringstream = IOStreamHandle(StringIO())
        self._testfile_stringstream = IOStreamHandle(StringIO(TEST_MODEL_PARAMETER_FORMATTER))

        self._roundtrip_streamreader = ParameterFormatterYamlReader(self._roundtrip_stringstream)
        self._roundtrip_streamwriter = ParameterFormatterYamlWriter(self._model_parameter_formatter, self._roundtrip_stringstream)
        ...
        self._testfile_streamreader_missing_keyword = ParameterFormatterYamlReader(self._testfile_stringstream_missing_keyword)
        self._testfile_streamreader_extra_keyword = ParameterFormatterYamlReader(self._testfile_stringstream_extra_keyword)
