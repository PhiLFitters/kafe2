...
class MinimizerROOTTMinuit(MinimizerBase):
    ...
    def _migrad(self, max_calls=6000):
        ...
        error_code = ctypes.c_int(0)
        self._get_gMinuit().mnexcm("MIGRAD", arr("d", [max_calls, self.tolerance]), 2, error_code)

    def _minuit_fcn(self, number_of_parameters, derivatives, f, parameters, internal_flag):
        """
        ...
        **internal_flag** : int
        ...
        parameter_list = np.frombuffer(parameters, dtype=float, count=self.num_pars)
    ...
    def unlimit(self, parameter_name):
        ...
        if self.__gMinuit is not None:
            # also update Minuit instance
            error_code = ctypes.c_int(0)
            self.__gMinuit.mnexcm("SET LIM", arr("d", [_par_id + 1]), 1, error_code)
            self._did_fit = False
        self._invalidate_cache()
