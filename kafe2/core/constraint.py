...
class GaussianSimpleParameterConstraint(ParameterConstraint):
    def __init__(self, index, value, uncertainty, relative=False):
        ...
        :type value: float
        :param uncertainty: The uncertainty with which the parameter should be constrained to the
            given value
    ...
    def cost(self, parameter_values):
        """
        Calculates additional cost depending on the fit parameter values.
        More specifically, the constraint first picks the value from ``parameter_values`` at
        ``self.index``.
        The constraint then calculates the residual by subtracting ``self.value``.
        The final cost is calculated by dividing the residual by ``self.uncertainty`` and squaring
        the result.
...
class GaussianMatrixParameterConstraint(ParameterConstraint):
    def __init__(
        ...
        :param matrix: The matrix that defines the correlation between the parameters. By default
        ...
        :param matrix_type: Whether the matrix should be interpreted as a covariance matrix or as
        ...
        if len(self._values.shape) != 1 or self._values.shape * 2 != _matrix_array.shape:
            raise ValueError(
                "Expected values and cov_mat to be of shapes (N, ), (N, N) but received "
                "shapes %s, %s instead!" % (self._values.shape, _matrix_array.shape)
            )
        if matrix_type == "cov":
            pass
    ...
    def uncertainties_rel(self):
        """the uncorrelated uncertainties for the parameters to be constrained to relative to
        ``self.values``"""
    ...
    @property
    def relative(self):
        """whether the constraint was initialized with a relative covariance matrix/with relative
        uncertainties"""
        return self._relative
    ...
    def cost(self, parameter_values):
        """
        Calculates additional cost depending on the fit parameter values.
        More specifically, the constraint first picks values from ``parameter_values`` according
        to ``self.indices``.
        The constraint then calculates the residuals by subtracting ``self.values``.
        The final cost is calculated by applying the residuals to both sides of
        ``self.cov_mat_inverse`` via dot product.
