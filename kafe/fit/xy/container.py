...
class XYContainer(IndexedContainer):
    ...
    def data(self, new_data):
        ...
        if _new_data.ndim != 2:
            raise XYContainerException("XYContainer data must be a 2-d array of floats! Got shape: %r..." % (_new_data.shape,))
        if _new_data.shape[0] == 2:
            ...
            raise XYContainerException(
                "XYContainer data length must be exactly 2 in at least one axis! Got shape: %r..." % (_new_data.shape,))
    ...
    def x(self, new_x):
        ...
        if len(_new_x_data.shape) > 1:
            raise XYContainerException("XYContainer 'x' data must be a 1-d array of floats! Got shape: %r..." % (_new_x_data.shape,))
