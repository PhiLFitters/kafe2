...
.. module:: kafe.fit
    ...
    :synopsis: This module contains specialized objects for storing measurement data,
               ...
               It relies on the :py:mod:`kafe.core` module for basic functionality.

.. moduleauthor:: Daniel Savoiu <daniel.savoiu@cern.ch>


The :py:mod:`kafe.fit` module provides an essential toolkit for estimating model
parameters from data ("fitting").

It distinguishes between a number of different data types:

  * series of indexed measurements (dedicated submodule: :py:mod:`~kafe.fit.indexed`),
  * *xy* data (dedicated submodule: :py:mod:`~kafe.fit.xy`), and
  * histograms (dedicated submodule: :py:mod:`~kafe.fit.histogram`)
...
The following objects are provided for handling *indexed* data, as described above:
  ...
  * :py:obj:`~kafe.fit.indexed.IndexedContainer`: data container for storing *indexed* data
  * :py:obj:`~kafe.fit.indexed.IndexedParametricModel`: corresponding model:
        uses a model function (:py:obj:`~kafe.fit.indexed.model.IndexedModelFunction`) to calculate the model
        predictions and stores the result in an :py:obj:`~kafe.fit.IndexedContainer`
  * :py:obj:`~kafe.fit.indexed.IndexedFit`: a fit of a parametric model to *indexed* data:
        finds the minimum of the *cost function* to find the
        parameter values for which the model best fits the data
  * :py:obj:`~kafe.fit.indexed.IndexedPlot`: a graphical representation of one or more fits:
...
The following objects are provided for handling *xy* data:
  ...
  * :py:obj:`~kafe.fit.xy.XYContainer`: data container for storing *xy* data
  * :py:obj:`~kafe.fit.xy.XYParametricModel`: corresponding model:
        uses a model function (:py:obj:`~kafe.fit.xy.model.XYModelFunction`) to calculate the model predictions and
        stores the result in an :py:obj:`~kafe.fit.XYContainer`
  * :py:obj:`~kafe.fit.xy.XYFit`: a fit of a parametric model to *xy* data:
        finds the minimum of the *cost function* to find the parameter values for which the model best fits the data
  * :py:obj:`~kafe.fit.xy.XYPlot`: a graphical representation of one or more fits:
        plots the data points and model predictions using ``matplotlib``
                                

Finally, *kafe* is also able to handle **histograms**. Histograms organize measurements whose
...
The following objects are provided for handling histograms:
  ...
  * :py:obj:`~kafe.fit.histogram.HistContainer`: data container for storing histograms
  * :py:obj:`~kafe.fit.histogram.HistParametricModel`: corresponding model:
        uses a model function (:py:obj:`~kafe.fit.xy.model.HistModelFunction`) to calculate the model predictions
        and stores the result in an :py:obj:`~kafe.fit.HistContainer`
  * :py:obj:`~kafe.fit.histogram.HistFit`: a fit of a parametric model to histograms:
        finds the minimum of the *cost function* to find the parameter values for which the model best fits the data
  * :py:obj:`~kafe.fit.histogram.HistPlot`: a graphical representation of one or more fits:
        plots the data points and model predictions using ``matplotlib``
"""
