...
class IndexedModelFunctionFormatter(ModelFunctionFormatter):
    def __init__(self, name, latex_name=None, x_name='i', latex_x_name='i',
    ...
    def get_formatted(self, with_par_values=True, n_significant_digits=2, format_as_latex=False, with_expression=False):
        ...
        if format_as_latex:
            _out_string = r"%s_{%s}\left(%s\right)" % (self._latex_name, self._latex_x_name, ", ".join(_par_strings))
            if _par_expr_string:
                _out_string += " = " + _par_expr_string
            _out_string = "$%s$" % (_out_string,)
        else:
            _out_string = "%s_%s(%s)" % (self._name, self._x_name, ", ".join(_par_strings))
            if _par_expr_string:
                _out_string += " = " + _par_expr_string
        return _out_string